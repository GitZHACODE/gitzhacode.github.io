---
title: "Parametric design thinking: A case-study of practice-embedded architectural research"
authors: Bhooshan, S.
year: 2017
keywords: parametric design, design research, research methods, digital design, computer aided design
link: "https://doi.org/10.1016/j.destud.2017.05.003"
image: /assets/images/publications/Parametric design thinking A case-study of practice-embedded architectural research.jpg
order: 17
abstract: >-
  The paper highlights aspects of a particular parametric design thinking (PDT) distilled from practice. It describes the components of PDT e cognitive model, design method and information processing model e that are critical to an efficacious, collaborative search for solutions to architectural problems. The aspect related to the information processing model is afforded a detailed examination, synthesising the state-of-the-art in practice and research. Lastly, case studies spanning six years trace the transfer of methods and knowledge from collaborations and prototypes into projects of ZHA Computation and Design group (ZHCODE). These exemplify the role of a shared language of geometry and several process related aspects of parametric design critical to its success.
citation: >-
  Bhooshan, S., 2017. Parametric design thinking: A case-study of practice-embedded architectural research. Design Studies, 52, pp.115-143.
---